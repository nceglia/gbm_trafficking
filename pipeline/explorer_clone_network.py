# %%
"""Build a single, fully self-contained interactive clone-network HTML
explorer at results/clone_network_explorer/index.html.

The HTML embeds all data (as JSON) and a vendored copy of D3 v7. No
network requests, no external assets at runtime. Aesthetic discipline
is enforced by pulling all colors and orderings from
pipeline.modules.style; phenotypes that don't map raise a KeyError at
build time.

Inputs:
  data/objects/GBM_TCR_POS_TCELLS.h5ad   (T cells; canonical file
       referenced by 06_branch_empirics — used here in place of the
       spec's "integrated_data_with_clones.h5ad", which is not present
       in this repo and refers to the same conceptual object)
  data/objects/MYELOID_GBM.h5ad
  results/06_branch_empirics/edge_metrics_table.csv

Outputs:
  results/clone_network_explorer/index.html
...
TCELL_PATH = REPO_ROOT / "data" / "objects" / "GBM_TCR_POS_TCELLS.h5ad"
MYELOID_PATH = REPO_ROOT / "data" / "objects" / "MYELOID_GBM.h5ad"
EDGE_METRICS_PATH = (REPO_ROOT / "results" / "06_branch_empirics"
                     / "edge_metrics_table.csv")
OUT_DIR = REPO_ROOT / "results" / "clone_network_explorer"
