...
We adopt a mean-field factorization $\vq(M,\xi) = \vq(M)\,\vq(\xi)$. The mean
...
\]
where both sums run over observations for which the tissue $s'$ of $z'$ is
...
\subsection{Estimated mean matrix}

After convergence, the fitted mean matrix is the variational posterior mean of
each Gamma factor,
\[
...
$\widehat{M}$ is the estimated one-step mean matrix over the $33$-state
