...
to sum to one (Section~\ref{sec:growth-factors}); the row sum is freed to carry
growth.

\subsection{Non-negative rows and growth factors}
\label{sec:growth-factors}
...
\]
is the net growth factor (per step) of source state $z$: $g_z > 1$ is net
...
% ======================================================================
\section{Bayesian Treatment of the Growth Matrix}
% ======================================================================
...
\begin{enumerate}[leftmargin=2em, itemsep=4pt]
  ...
  \item \textbf{Observation.} For each destination state $z'$ whose tissue is
        non-missing ($m_{j,s'} = 1$), draw the destination count
        \[
...
\bottomrule
\end{tabular}
\caption{Per-step read-outs of the fitted growth matrix $\widehat{M}$.}
\label{tab:readouts}
\end{table}

Here $T$ and the per-state growth factor $g_z$ are the factors of
$\widehat{M}=\operatorname{diag}(g)\,T$ from Section~\ref{sec:growth-factors},
