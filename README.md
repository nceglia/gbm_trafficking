...
The dependency graph and lineage notes live under [`pipeline/`](pipeline/):

- [`pipeline/manifest.yaml`](pipeline/manifest.yaml) — machine-readable DAG
- [`pipeline/dag.md`](pipeline/dag.md) — auto-generated graph (`python pipeline/workflow/render_from_manifest.py`)
- [`pipeline/AUDIT.md`](pipeline/AUDIT.md) — audit summary (TCR vs myeloid, Bayesian coupling)
- [`workflow/README.md`](workflow/README.md) — optional Snakemake orchestration (local or SLURM)

## Pipeline viewers (VM deploy)

Static HTML explorers are built on the analysis machine and synced to a small VM.
